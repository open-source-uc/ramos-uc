...
def planner(request):
    data = cache.get_or_set('planner_data', {
...
def share(request):
    return render(request, 'courses/share.html', {
        'mods': ['8:30', '10:00', '11:30', '2:00', '3:30', '5:00', '6:30', '8:00'],
...
def single_course(request, initials):
    ...
    if cached_course is None:
        ...
        # Link requirements
        requirements = re.sub(r'([a-zA-Z]{3}\d{3,4}[a-zA-Z]?)',
                              r'<a href="/ramo/\1">\1</a>', course.req)
        cached_course = {
            'course': course,
...
def browse(request):
    ...
    if school_name is not None:
        ...
        return render(request, 'courses/school.html', {
            'courses_page': paginator.get_page(page_number),
            'school_name': school_name,
        })

    # Case all schools
    cached_schools = cache.get_or_set('all_schools', {
                'schools': Course.objects.available('school')
            }, 3600*24)
