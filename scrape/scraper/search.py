...
class _BCParser(HTMLParser):
    ...
    def process_course(self):
        ...
        course['schedule'] = course['schedule'].replace('<table>', '').replace('</table>', '')
        course['schedule'] = course['schedule'].replace('<img>', '').replace('</img>', '')

        # Turn profesor Apellido Nombre to Nombre Apellido
        if course['teachers'] not in ['Dirección Docente', '(Sin Profesores)', 'Por Fijar']:
